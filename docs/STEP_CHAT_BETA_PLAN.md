...
Trotter opens as an authenticated standalone page at `/chat` from the simple **Trotter (beta)** button beside **Save Steps**. Keeping chat in its own document avoids mobile keyboard and nested-scroll conflicts with the dashboard while preserving the browser’s normal Back behavior.
It supports:
