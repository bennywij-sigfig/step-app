...
describe('Trotter standalone mobile layout contract', () => {
  test('uses an authenticated standalone page instead of a dashboard overlay', () => {
    ...
    expect(dashboard).not.toContain('/chat-v2');
    expect(js).toContain("const API_BASE = IS_CHAT_V2 ? '/api/chat-v2' : '/api/chat'");
  ...
  test('makes batch date acknowledgement and add/replace consequences explicit', () => {
    ...
    expect(js).toContain("postJson(`${API_BASE}/cancel`, { plan_id: result.plan_id })");
  ...
  test('uses versioned chat assets and responsive desktop/mobile shells', () => {
    ...
    expect(css).toMatch(/\.chat-composer textarea::placeholder\s*\{[\s\S]*?font-size: 13px/);
