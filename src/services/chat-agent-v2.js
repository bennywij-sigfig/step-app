...
async function runNativeTrotterAgent({ model, registry, message, history, tone, context }) {
  const toolResults = [];
  ...
  for (let turn = 1; turn <= MAX_MODEL_TURNS; turn += 1) {
    ...
    if (calls.length === 0) {
      ...
      if (!text) {
        throw new ChatAgentProtocolError('Model returned neither text nor a tool call', { turn });
      }
