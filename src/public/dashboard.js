    ...
    async function getCSRFToken() {
        ...
        function createUserDataList(userId, userName, userData) {
            ...
            if (userData.daily_steps.length === 0) {
                ...
                // Calculate challenge duration if active challenge exists
                let challengeDays = 14; // Default fallback
                ...
                if (currentUser && currentUser.current_challenge) {
                    ...
                    // Calculate challenge duration (inclusive of end date)
                    challengeDays = Math.floor((endDate - startDate) / (1000 * 60 * 60 * 24)) + 1;
                    showingText = userData.daily_steps.length > challengeDays ? ` (showing latest ${challengeDays} of ${userData.total_days} days)` : ` (${userData.total_days} days total)`;
                }
                
                const dailyDataHtml = userData.daily_steps.slice(0, challengeDays).map((day, index) => `
